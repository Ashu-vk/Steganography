...
public class JsonToExcelConverter {
	public static void main(String[] args) {
		String jsonFilePath = "D:\\steganography\\abc.json"; // Path to your JSON file
		String excelFilePath = "D:\\steganography\\bankTransaction.xlsx"; // Output Excel file
...
//			Sheet sheet = workbook.createSheet("CreditNotes");
			...
			JsonNode bankTxns = objectMapper.readTree(new File(jsonFilePath));
//			JsonNode creditNotes = objectMapper.readTree(new File(jsonFilePath));
			...
			String[] paymentHeaders = { "BankTransactionID", "BankAccount", "Type", "Reference", "IsReconciled", "Contact",
					"DateString", "Status", "LineAmount", "AccountCode", "Total" };
...
//					"Payments", "Contact", "InvoiceAddresses", "LineItems" };
			...
			for (int i = 0; i < paymentHeaders.length; i++) {
				headerRow.createCell(i).setCellValue(paymentHeaders[i]);
			}

			// Iterate through JSON array
			for (JsonNode payment : bankTxns) {
				Row row = sheetPayments.createRow(rowNum++);

				row.createCell(0).setCellValue(getJsonValue(payment, "BankTransactionID"));
				row.createCell(1).setCellValue(getJsonNodeValue(payment.get("BankAccount"), "Name"));
				row.createCell(2).setCellValue(getJsonValue(payment, "Type"));
				row.createCell(3).setCellValue(getJsonValue(payment, "Reference"));
				row.createCell(4).setCellValue(getJsonValue(payment, "IsReconciled"));
				row.createCell(5).setCellValue(getJsonNodeValue(payment.get("Contact"), "Name"));
				row.createCell(6).setCellValue(getJsonValue(payment, "DateString"));
				row.createCell(7).setCellValue(getJsonValue(payment, "Status"));
				row.createCell(8).setCellValue(getFromLineItem(payment.get("LineItems"),"LineAmount"));
				row.createCell(9).setCellValue(getFromLineItem(payment.get("LineItems"),"AccountCode"));
				row.createCell(10).setCellValue(getJsonValue(payment, "Total"));
